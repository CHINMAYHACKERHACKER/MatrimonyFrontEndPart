...
import { Box, Button, Card, CardContent, Container, FormHelperText, Grid, IconButton, InputAdornment, Stack, TextField, Typography } from '@mui/material';
...
const Reset = () => {
  ...
  const getdata = async () => {
    try {
      console.log(token);
      const res = await APIservice.httpPost('/api/admin/users/verifyforgotPasswordLink', { token });
  ...
  const mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{7,})");
  ...
  const handlesubmit = async (event: any) => {
    ...
    if (flag) {
      try {
        ...
        const res = await APIservice.httpPost('/api/admin/users/resetPassword', obj);
            ...
            <Card sx={{ borderRadius: "10px" }}>
              <CardContent style={{ padding: "24px" }}>
                <Form onSubmit={onFormSubmit}>
                  <Typography align="center" fontSize="25px" fontWeight="bolder" className="mb-3">Reset Password</Typography>
                  <TextField
                      required
                      sx={{ width: "100%" }}
                      label="New Password"
                  ...
                  <Stack direction="row" alignItems="center" justifyContent="flex-end" sx={{ my: 1 }}>
                    <RouterLink to="/" className="forgotPasswordFont" style={{ color: "#5569ff", textDecoration: "none", justifyContent: "flex-end", fontSize: "15px" }}>Back to login page?</RouterLink>
                  </Stack>
                </Form>
              </CardContent>
            </Card>
