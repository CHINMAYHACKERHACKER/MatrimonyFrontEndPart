...
import { Card, FormControl, Grid, Container, Box, Breadcrumbs, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, FormHelperText, IconButton, Stack, styled, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Tooltip, Typography, useTheme, FormGroup, Avatar, InputAdornment, Paper } from '@mui/material';
...
function Occupation() {
  ...
  const handleClickisEdit = (element: any) => {
    ...
    if (element.imageUrl) {
      setImage(process.env.REACT_APP_IMAGE_URL + element.imageUrl)
    } else {
      setImage("")
    }
    setOccupation(obj);
  ...
  const handleSwitchCheck = async () => {
    ...
    const res = await APIservice.httpPost('/api/admin/occupation/activeInactiveOccupation', obj, token, refreshToken);
    setIsCheck(false);
  ...
  const getData = async (startIndex: number, fetchRecord: number) => {
    try {
      if (search) {
        ...
        const res = await APIservice.httpPost('/api/admin/occupation/getOccupation', obj, token, refreshToken);
    ...
    if (flag) {
      try {
        if (occupation.id) {
          const token = localStorage.getItem('SessionToken');
          const refreshToken = localStorage.getItem('RefreshToken')
          let val = { "id": occupation.id, "name": occupation.name, "imageUrl": image }
          const res = await APIservice.httpPost('/api/admin/occupation/insertUpdateOccupation', val, token, refreshToken);
          if (res && res.status == 200) {
            console.log("success");
            getData((page * limit), limit);
            setIsOpen(false);
            ...
            if (!(flag && errorFlag)) {
              setUserError(true)
              setUserErrorMsg('Occupation already exists!')
              setErrorFlag(true)
            }
          } else if (res.status == 500) {
            setIsOpen(false);
          ...
          const res = await APIservice.httpPost('/api/admin/occupation/insertUpdateOccupation', occupation, token, refreshToken);
          if (res && res.status == 200) {
            console.log("success");
            setPage(0);
            setIsOpen(false);
            ...
            <Grid item >
              <Stack alignItems="left" justifyContent="space-between" >
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography variant="subtitle2" color="inherit" fontWeight="bold">Occupation</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <Grid item>
            ...
            <>
                                          ...
                                          >
                                            ...
                                            <TableCell >
                                              <Stack direction="row" alignItems="center" spacing={2}>
                                                {arr.imageUrl ? <Avatar src={process.env.REACT_APP_IMAGE_URL + arr.imageUrl} alt="occupation"></Avatar> : <Avatar>{arr.name ? arr.name[0] : null}</Avatar>}
                                                <Typography
                                                  variant="body1"
                                                  fontWeight="bold"
                                                  color="text.primary"
                                            ...
                                            <TableCell align='right' sx={{ display: "flex", padding: "20px" }}>
                                              ...
                                              <Tooltip title="Edit " arrow>
                                                <IconButton
                                                  ...
                                                  color="inherit"
                                                  size="small"
                                                  onClick={(e) => handleClickisEdit(arr)}
                                                >
              ...
              <div>
                <Dialog open={ischeck} onClose={handleClose} fullWidth maxWidth="xs">
                  <DialogTitle sx={{ m: 0, p: 2, fontSize: "20px", fontWeight: "bolder" }}>{(occupation.status === 0) ? 'Inactive' : 'Active'}</DialogTitle>
                  <DialogContent>
                    <DialogContentText style={{ fontSize: "1rem", letterSpacing: "0.00938em" }}>
                      {(occupation.status === 0) ? 'Are you sure you want to Active?' : 'Are you sure you want to Inactive?'}
                    </DialogContentText>
                  </DialogContent>
                  <DialogActions>
              ...
              <>
                <BootstrapDialog open={isOpen} onClose={handleCloseOccupationDailog} PaperProps={{ sx: { height: "40%" } }} fullWidth maxWidth="xs">
                  <BootstrapDialogTitle id="customized-dialog-title" onClose={handleCloseOccupationDailog}>
                  ...
                  <DialogContent dividers>
                    <Form >
                      ...
                      <TextField
                        ...
                        onChange={(arr) => { OccupationDialog(arr); validateName(arr) }}
                        required={true}
                      />
                  ...
                  <Box sx={{ display: "flex", justifyContent: "space-between", p: "8px" }}>
                    ...
                    <Button onClick={saveOccupation}>Save</Button>
                  </Box>
                </BootstrapDialog>
              </>
            </>
