...
const ReportRejectUser = () => {
  ...
  const getData = async (startIndex: number, fetchRecord: number) => {
    ...
    const res = await APIservice.httpPost('/api/admin/report/getMonthlyRejectProposalUser', obj, token, refreshToken);
  ...
  const handlePrint = async () => {
    const token = localStorage.getItem('SessionToken');
    const refreshToken = localStorage.getItem('RefreshToken');
    const res = await APIservice.httpPost("/api/admin/report/getMonthlyRejectProposalUser", userValue, token, refreshToken);
    if (res && res.status == 200) {
      console.log("success");
    }
    else if (res.status == 401) {
...
<div>`
    html +=
      (userValue.fromDate ? `<p>` + (format(new Date(userValue.fromDate), dateData)) + `</p>
  </div>
  <div>
  <p>    -    </p>
  </div>
  <div>`
        ...
        '')
    html +=
      `<p>` + (userValue.toDate ? (format(new Date(userValue.toDate), dateData)) : '') + `</p>
  </div>
</div>
  ...
  .td4 {
    ...
    for (let i = 0; i < res.recordList.length; i++) {
      ...
      html +=
        `<tr>
          <td class="td1">`+ (i + 1) + `</td>
          <td class="td2">`+ res.recordList[i].userName + ` </td>
         <td class="td3">` + res.recordList[i].proposalName + ` </td>
         <td class="td4">` + format(new Date(res.recordList[i].createdDate), dateData) + ` </td>
         </tr>`
    }
    html += `</tbody>
      </table>
        ...
        <Stack spacing={3} sx={{ m: 2, backgroundColor: 'white' }}>
          <FormGroup>
            <FormControl fullWidth className="mb-3">
              <InputLabel htmlFor="demo-customized-select-label">Year</InputLabel>
              <Select
              ...
              >
                {year.map((arr: any) => (
                  <MenuItem key={arr.data} value={arr.data}>{arr.data}</MenuItem>
                ))}
              </Select>
            </FormControl>
            <FormControl fullWidth className="mb-3">
              <InputLabel htmlFor="demo-customized-select-label">Month</InputLabel>
              <Select
                labelId="demo-customized-select-label"
                id="demo-customized-select"
                name="month"
            ...
            <Grid item >
              ...
              <Stack alignItems="left" justifyContent="space-between">
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography color="inherit" variant="subtitle2" fontWeight="bold">Proposal Reject User</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <div>
            ...
            <Grid item>
              ...
              <Tooltip title="Export File" arrow>
                <IconButton
                  ...
                  size="small"
                  onClick={DataSend}>
                  <CSVLink data={detail} filename={"ReportSend.csv"} style={{
                    '&:hover': { background: theme.colors.primary.lighter },
                    color: theme.palette.primary.main,
                        ...
                        <>
                          <TableContainer className='userProposalTableContainer'>
                            <Table stickyHeader>
                              <TableHead>
                                <TableRow>
                                  <TableCell align='center'  ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Id</Typography></TableCell>
                                  {/* <TableCell align='center'>Month</TableCell> */}
                                  <TableCell align='center'><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Proposal Sender</Typography></TableCell>
                                  <TableCell align='center'  ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>proposal Receiver</Typography></TableCell>
                                  <TableCell align='center' ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Date</Typography></TableCell>
                                </TableRow>
                              </TableHead>
                              <TableBody>
                                <>
                                      ...
                                      >
                                        ...
                                        <TableCell align='center' >
                                          <Typography
                                            ...
                                            noWrap
                                          >
                                            {arr.proposalName ? arr.proposalName : "--"}
                                          </Typography>
                                        </TableCell>
                                        <TableCell align='center' >
                                          <Typography
