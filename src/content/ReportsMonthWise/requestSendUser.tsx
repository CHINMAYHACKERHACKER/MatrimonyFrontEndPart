...
const ReportSendUser = () => {
  ...
  const getData = async (startIndex: number, fetchRecord: number) => {
    ...
    const res = await APIservice.httpPost('/api/admin/report/getMonthlySendProposalUser', obj, token, refreshToken);
  ...
  const handlePrint = async () => {
    const token = localStorage.getItem('SessionToken');
    const refreshToken = localStorage.getItem('RefreshToken');
    const res = await APIservice.httpPost("/api/admin/report/getMonthlySendProposalUser", userValue, token, refreshToken);
    if (res && res.status == 200) {
      console.log("success");
    }
    else if (res.status == 401) {
            ...
            <Grid item >
              <Stack alignItems="left" justifyContent="space-between">
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography color="inherit" variant="subtitle2" fontWeight="bold">Proposal Send User</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <div>
