...
import { Box, Grid, Breadcrumbs, Typography, Stack, Button, Container, Card, TableContainer, TablePagination, Divider, IconButton, Switch, Tooltip, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, TextField, Accordion, AccordionSummary, AccordionDetails, FormHelperText, Paper } from '@mui/material';
...
export default function Question() {
    ...
    const handleClickisEdit = (no: number, que: string, ans: string, name: any) => {
        let obj = {
            id: no,
            question: que,
            answer: ans,
    ...
    const handleSwitchCheck = async () => {
        ...
        const res = await APIservice.httpPost('/api/admin/questionCategories/activeInactiveQuestion', obj, token, refreshToken);
        setIsCheck(false);
        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
    }
    ...
    const handleDelete = async () => {
        ...
        const res = await APIservice.httpPost('/api/admin/questionCategories/deleteQuestion', obj, token, refreshToken);
        setIsDel(false);
        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
    }
    ...
    const getData = async (startIndex: number, fetchRecord: number, startIndexCategories: number, fetchRecordCategories: number) => {
        try {
            setIsLoading(true)
            const token = localStorage.getItem('SessionToken');
            const refreshToken = localStorage.getItem('RefreshToken');
            ...
            const res = await APIservice.httpPost('/api/admin/questionCategories/getQuestion', obj, token, refreshToken);
        ...
        try {
            ...
            if (flag) {
                if (question.id) {
                    ...
                    const res = await APIservice.httpPost('/api/admin/questionCategories/insertUpdateQuestion', val, token, refreshToken)
                    if (res && res.status === 200) {
                        console.log("success")
                        setIsOpen(false);
                        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
                    ...
                    const res = await APIservice.httpPost('/api/admin/questionCategories/insertUpdateQuestion', obj, token, refreshToken)
                    if (res && res.status == 200) {
        ...
        try {
            ...
            if (flag) {
                if (question.id) {
                    ...
                    const res = await APIservice.httpPost('/api/admin/questionCategories/insertUpdateQuestionCategories', val, token, refreshToken)
                    if (res && res.status === 200) {
                        console.log("success")
                        setOpen(false)
                        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
                    ...
                    const res = await APIservice.httpPost('/api/admin/questionCategories/insertUpdateQuestionCategories', obj, token, refreshToken)
                    if (res && res.status === 200) {
                        console.log("success")
                        setOpen(false)
                        getData(0, limit, 0, limitCategories)
    ...
    const handleSwitchCheckCategories = async () => {
        ...
        const res = await APIservice.httpPost('/api/admin/questionCategories/activeInactiveQuestionCategories', obj, token, refreshToken);
        setCheck(false);
        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
    }
    ...
    const handleDeleteCategories = async () => {
        ...
        const res = await APIservice.httpPost('/api/admin/questionCategories/deleteQuestionCategories', obj, token, refreshToken);
        setDel(false);
        getData((page * limit), limit, (pageCategories * limitCategories), limitCategories);
    }
    ...
    const handlePageChange = (event: any, newPage: number): void => {
        setPage(newPage);
        getData((newPage * limit), limit, (pageCategories * limitCategories), limitCategories);
    };
    ...
    const handleCategoriesLimitChange = (event: ChangeEvent<HTMLInputElement>): void => {
        ...
        <>
                        ...
                        <Grid item>
                            <Stack alignItems="left" justifyContent="space-between">
                                <Breadcrumbs aria-label="breadcrumb">
                                    ...
                                    <Typography variant="subtitle2" color="inherit" fontWeight="bold" >FAQs</Typography>
                                </Breadcrumbs>
                            </Stack>
                        </Grid>
                        <Grid item>
                                                ...
                                                <TableContainer className="apptableContainer">
                                                    {user.map((arr: any, index: number) => (
                                                        <Accordion key={index} sx={{ mt: 1 }}>
                                                            <AccordionSummary
                                                                expandIcon={<ExpandMoreIcon sx={{ color: "#495fff" }} />}
                                                                aria-controls="panel1a-content"
                                                                id="panel1a-header"
                                                                sx={{ bgcolor: "#c7cdfd", display: "flex", justifyContent: "space-between" }}
                                                            >
                                                                <Grid container justifyContent="space-between">
                                                                    <Grid item sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                                        <Typography sx={{ fontSize: "17px", fontWeight: "bold", textTransform: "capitalize", color: "#495fff" }}>{arr.name}</Typography>
                                                                    </Grid>
                                                                    <Grid item sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                                                        <Tooltip title={(arr.isActive === 0) ? "Inactive" : "Active"} arrow>
                                                                            <Switch
                                                                                checked={arr.isActive === 0 ? false : true}
                                                                                onClick={(e) => handleSwitchCategories(arr.id, arr.isActive)}
                                                                        ...
                                                                        <Tooltip title="Edit " arrow>
                                                                            <Button
                                                                                ...
                                                                                size="small"
                                                                                variant="contained"
                                                                                onClick={(e) => handleEditCategoryName(arr.id, arr.name)}
                                                                            >
                                                                        ...
                                                                        <Tooltip title="Add " arrow>
                                                                            <Button
                                                                                ...
                                                                                size="small"
                                                                                variant="contained"
                                                                                onClick={(e) => { handleClickisAdd(arr.id, arr.name) }}
                                                                            >
                                                                                <AddTwoToneIcon fontSize="small" />
                                                                            </Button>
                                                                        </Tooltip>
                                                                        <Tooltip title="Delete" arrow>
                                                                            <Button
                                                                                ...
                                                                                size="small"
                                                                                variant="contained"
                                                                                onClick={(e) => handleDeleteCategoriesDialog(arr.id)}
                                                                            >
                                                            ...
                                                            {arr.question.map((data: any, index: number) => (
                                                                <>
                                                                    <AccordionDetails key={index}>
                                                                        <Box sx={{ display: "flex", justifyContent: "space-between", bgcolor: "aliceblue", alignItems: "center" }}>
                                                                            <Typography sx={{ fontSize: { lg: "18px", md: "17px", sm: "16px", xs: "15px" }, fontWeight: "bold", p: 1 }}>{data.question}</Typography>
                                                                            <div style={{ padding: "9px", display: "flex" }}>
                                                                                ...
                                                                                <Tooltip title="Edit " arrow>
                                                                                    <Button
                                                                                        ...
                                                                                        size="small"
                                                                                        variant="contained"
                                                                                        onClick={(e) => handleClickisEdit(data.id, data.question, data.answer, arr.name)}
                                                                                    >
                                                                                        <EditTwoToneIcon fontSize="small" />
                                                                                    </Button>
                                                                                </Tooltip>
                                                                                <Tooltip title="Delete" arrow>
                                                                                    <Button
                                                                                        ...
                                                                                        size="small"
                                                                                        variant="contained"
                                                                                        onClick={(e) => handleClickisDelete(data.id)}
                                                                                    >
                                                                                        <DeleteIcon fontSize="small" />
                                                                                    </Button>
                                                                                </Tooltip>
                                                                        ...
                                                                        <Typography sx={{ fontSize: "16.5px", p: 1 }} dangerouslySetInnerHTML={{ __html: data.answer }} />
                                                                    </AccordionDetails>
                                                                </>
                                                            ))}
                                                            <Divider />
            ...
            <div>
                <Dialog open={ischeck} onClose={handleClose} fullWidth maxWidth="xs">
                    <DialogTitle sx={{ m: 0, p: 2, fontSize: "20px", fontWeight: "bolder" }}>
                        {(question.status === 0) ? 'Inactive' : 'Active'}
                    </DialogTitle>
                    <DialogContent>
                        <DialogContentText style={{ fontSize: "1rem", letterSpacing: "0.00938em" }}>
                            {(question.status === 0) ? 'Are you sure you want to Active?' : 'Are you sure you want to Inactive?'}
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions>
            ...
            <div>
                <Dialog open={isOpen} onClose={handleClose} fullWidth>
                    <DialogTitle sx={{ m: 0, p: 2, fontSize: "18px", fontWeight: "bold" }}>
                        {question.id ? "Edit Question" : "Add Question"} {"(" + question.name + ")"}
                        <IconButton
                            aria-label="close"
                            onClick={handleClose}
                            sx={{
                    ...
                    <DialogContent dividers>
                        <FormControl fullWidth>
                            ...
                            <TextField
                                ...
                                onChange={(e) => { handleChange(e); validateQuestion(e) }}
                                required={true}
                            />
                        </FormControl>
                        <FormHelperText style={{ color: 'red', height: '22px' }}>{isQuestionError && QuestionErrorMsg}</FormHelperText>
                        <FormControl sx={{ mt: 2 }} fullWidth>
                            ...
                            <div style={{ marginTop: "9px" }}>
                                <JoditEditor
                                    ...
                                    // onBlur={(newContent) => setQuestion(newContent.target.innerHTML)}// preferred to use only this option to update the content for performance reasons
                                    onChange={(e: any) => { handleChange1(e); validateAnswer(e) }}
                                />
                            </div>
                            <FormHelperText style={{ color: 'red', height: '22px' }}>{isAnswerError && AnswerErrorMsg}</FormHelperText>
                        </FormControl>
                    </DialogContent>
            ...
            <div>
                <Dialog open={check} onClose={handleClose} fullWidth maxWidth="xs">
                    ...
                    <DialogActions>
                        <Button onClick={handleClose}>Cancel</Button>
                        <Button onClick={handleSwitchCheckCategories}>Yes</Button>
                    </DialogActions>
                </Dialog>
            </div>
            <div>
                <Dialog open={Del} onClose={handleClose} fullWidth maxWidth="xs">
                    ...
                    <DialogActions>
                        <Button onClick={handleClose}>Cancel</Button>
                        <Button onClick={handleDeleteCategories}>Yes</Button>
                    </DialogActions>
                </Dialog>
            </div>
            <div>
                <Dialog open={open} onClose={handleClose} PaperProps={{ sx: { height: "40%" } }} fullWidth maxWidth="xs">
                    <DialogTitle sx={{ m: 0, p: 2, fontSize: "18px", fontWeight: "bold" }}>
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", padding: "8px" }}>
                        <FormHelperText style={{ color: 'red', height: '22px', margin: 'none', padding: "8px 0px" }}>{isUserError && UserErrorMsg}</FormHelperText>
                        <Button onClick={saveQuestionCategories}>Save</Button>
                    </Box>
                </Dialog>
            </div>
        </>
