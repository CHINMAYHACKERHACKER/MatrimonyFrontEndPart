...
import { alpha, Avatar, Box, Breadcrumbs, Button, Card, CardActions, CardContent, CardHeader, Chip, Container, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Grid, IconButton, Paper, Stack, styled, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tooltip, Typography, useTheme } from '@mui/material';
...
function Dashboard() {
  ...
  const [monthlyRegistration, setMonthlyRegistration] = React.useState<number>(0);
  ...
  const getdata = async () => {
    try {
      ...
      const res = await APIservice.httpPost("/api/admin/dashboard/getDashboardData", obj, token, refreshToken);
      setUser(res.recordList[0].recentUserResult);
      setMonthlyProposal(res.recordList[0].monthlyProposal);
      setMonthlyRegistration(res.recordList[0].monthlyRegistration);
      setTodayProposal(res.recordList[0].todayProposal);
      setTodayRegistration(res.recordList[0].todayRegistration);
      setUserData(res.recordList[0].monthlyRegUserCount.map((item: any) => item.count))
      setMonth(res.recordList[0].monthlyRegUserCount.map((item: any) => item.month))
  ...
  const getProposal = async () => {
    try {
      ...
      const res = await APIservice.httpPost('/api/admin/report/getReceiveProposalReqReport', obj, token, refreshToken);
      setAccept(res.recordList.map((item: any) => item.count))
      const res1 = await APIservice.httpPost('/api/admin/report/getSendProposalReqReport', obj, token, refreshToken);
      setSend(res1.recordList.map((item: any) => item.count))
      const res2 = await APIservice.httpPost('/api/admin/report/getRejectProposalReqReport', obj, token, refreshToken);
  ...
  const getPendingBlockUser = async () => {
    ...
    try {
      ...
      const res = await APIservice.httpPost('/api/admin/userBlockRequest/getUserBlockRequest', obj, token, refreshToken);
            ...
            <Grid item>
              <Stack alignItems="left" justifyContent="space-between">
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography variant="subtitle2" color="inherit" style={{ fontWeight: "bold" }}>Dashboard</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <div>
              {chip === true ?
                <>
                  <Chip label={format(new Date(userValue.fromDate), dateData) + "-" + format(new Date(userValue.toDate), dateData)}
            ...
            <>
              <Grid container spacing={3}>
                <Grid xs={12} sm={6} md={3} item>
                  <Card
                    sx={{
                      px: 1, height: "201.17px"
                    }}
                  >
                    {isLoading ? <LoaderSmallCard title="Loading..." /> :
                      <>
                        <Link
                          to="/admin/appuser"
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={6} md={6}>
                    <Card style={{ height: "439.66px" }}>
                      {isLoading ? <LoaderGraph title="Loading..." /> :
                        <>
                          <CardHeader
                            title={
                              <Link
                    ...
                    <Card style={{ height: "439.66px" }}>
                      {isLoading ? <LoaderGraph title="Loading..." /> :
                        <>
                          ...
                          />
                          <Chart options={chartOptions} type="line"
                            series={series1}
                          />
                        </>
                      }
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={6} md={6}>
                    <Card style={{ height: "514px" }}>
                      {isLoading ? <LoaderCard title="Loading..." /> :
                        <>
                          <CardHeader
                            title={
                              <Link
                          ...
                          {blockUser && blockUser.length > 0 ?
                            <TableContainer style={{ height: "459px" }}
