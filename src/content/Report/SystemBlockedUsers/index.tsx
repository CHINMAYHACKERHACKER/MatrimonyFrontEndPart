import { Box, Breadcrumbs, Button, Card, Chip, Container, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, Grid, IconButton, InputAdornment, Paper, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Tooltip, Typography, useTheme } from '@mui/material'
...
const SystemBlockedUsers = () => {
  ...
  const getData = async (startIndex: number, fetchRecord: number) => {
    try {
      ...
      const res = await APIservice.httpPost("/api/admin/report/getSystemBlockedUsers", obj, token, refreshToken);
  ...
  const handlePrint = async () => {
    const token = localStorage.getItem('SessionToken');
    const refreshToken = localStorage.getItem('RefreshToken');
    const res = await APIservice.httpPost("/api/admin/report/getSystemBlockedUsers", userValue, token, refreshToken);
    ...
    for (let i = 0; i < res.recordList.length; i++) {
      html +=
        `<tr>
         ...
         <td class="td4">` + res.recordList[i].contactNo + ` </td>
         <td class="td5">`+ res.recordList[i].gender + ` </td>
         <td class="td6">`+ format(new Date(res.recordList[i].modifiedDate), dateData) + ` </td>
         </tr>`
    }
    html += `</tbody>
      </table>
            ...
            <Grid item>
              <Stack alignItems="left" justifyContent="space-between" >
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography color="inherit" variant="subtitle2" fontWeight="bold">System Blocked User</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <div>
            ...
            <Grid item>
              ...
              <Tooltip title="Export File" arrow>
                <IconButton
                  ...
                  onClick={DataSend}
                >
                  <CSVLink data={data1} filename={"SystemBlockedUser.csv"} style={{
                    '&:hover': { background: theme.colors.primary.lighter },
                    color: theme.palette.primary.main,
                            ...
                            <Table stickyHeader>
                              <TableHead>
                                <TableRow>
                                  ...
                                  <TableCell ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>ContactNo</Typography></TableCell>
                                  <TableCell ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Gender</Typography></TableCell>
                                  <TableCell ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Modified Date</Typography></TableCell>
                                </TableRow>
                              </TableHead>
                              <TableBody>
                                {
                                        ...
                                        >
                                          ...
                                          <TableCell >
                                            <Typography
                                              ...
                                              noWrap
                                            >
                                              {format(new Date(arr.modifiedDate), dateData)}
                                            </Typography>
                                          </TableCell>
                                        </TableRow>
                                      );
