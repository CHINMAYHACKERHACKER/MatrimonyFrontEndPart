import { Box, Breadcrumbs, Card, Container, Divider, Grid, IconButton, Paper, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, Tooltip, Typography, useTheme } from '@mui/material'
...
const RequestReceive = () => {
    ...
    const getdata = async (startIndex: number, fetchRecord: number) => {
        try {
            ...
            const res = await APIservice.httpPost("/api/admin/report/getTopProposalReceiveReqReport", obj, token, refreshToken);
    ...
    //     },
    //     {
    //         label: "Request Receive", key: "receiveRequest"
    //     },
    //     {
    ...
    const handlePrint = async () => {
        const token = localStorage.getItem('SessionToken');
        const refreshToken = localStorage.getItem('RefreshToken');
        const res = await APIservice.httpPost("/api/admin/report/getTopProposalReceiveReqReport", {}, token, refreshToken);
    ...
    <tbody>`
        for (let i = 0; i < res.recordList.length; i++) {
          ...
          <td class="td3">`+ res.recordList[i].receiveRequest + ` </td>
         <td class="td4">` + res.recordList[i].gender + ` </td>
         </tr>`
        }
        html += `</tbody>
      </table>
                        ...
                        <Grid item>
                            <Stack alignItems="left" justifyContent="space-between" >
                                <Breadcrumbs aria-label="breadcrumb">
                                    ...
                                    <Typography color="inherit" variant="subtitle2" fontWeight="bold">Top Requests Received</Typography>
                                </Breadcrumbs>
                            </Stack>
                        </Grid>
                        <Grid item>
                            ...
                            <Tooltip title="Export File" arrow>
                                <IconButton
                                    ...
                                    onClick={DataSend}
                                >
                                    <CSVLink data={data1} filename={"RequestReceiveReport.csv"} style={{
                                        '&:hover': { background: theme.colors.primary.lighter },
                                        color: theme.palette.primary.main,
                                                    ...
                                                    <Table stickyHeader>
                                                        <TableHead>
                                                            <TableRow>
                                                                ...
                                                                <TableCell ><Typography noWrap style={{ fontSize: "13px", fontWeight: "bold", marginBottom: "none" }}>Request Receive</Typography></TableCell>
