...
function BootstrapDialogTitle(props: DialogTitleProps) {
  ...
  return (
    <DialogTitle sx={{ m: 0, p: 2, fontSize: "20px", fontWeight: "bold" }} {...other}>
      {children}
      {onClose ? (
        <IconButton
          aria-label="close"
...
function MaritalStatus() {
  ...
  const handleSwitchCheck = async () => {
    ...
    const res = await APIservice.httpPost('/api/admin/maritalStatus/activeInactiveMaritalStatus', obj, token, refreshToken);
    setIsCheck(false);
  ...
  const getdata = async (startIndex: number, fetchRecord: number) => {
    if (search) {
      ...
      const res = await APIservice.httpPost("/api/admin/maritalStatus/getMaritalStatus", obj, token, refreshToken);
    ...
    if (flag) {
      try {
        if (v1.id) {
          ...
          const res = await APIservice.httpPost('/api/admin/maritalStatus/insertUpdateMaritalStatus', val, token, refreshToken);
          if (res && res.status == 200) {
            console.log("success");
            getdata((page * limit), limit);
            setIsOpen(false);
            ...
            if (!(flag && errorFlag)) {
              setUserError(true)
              setUserErrorMsg('Marital Status already exists!')
              setErrorFlag(true)
            }
          }
        }
          ...
          const token = localStorage.getItem('SessionToken');
          const refreshToken = localStorage.getItem('RefreshToken');
          const res = await APIservice.httpPost('/api/admin/maritalStatus/insertUpdateMaritalStatus', v1, token, refreshToken);
            ...
            <Grid item>
              <Stack alignItems="left" justifyContent="space-between" >
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography variant="subtitle2" color="inherit" fontWeight="bold" >Marital Status</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <Grid item>
              <Grid container spacing={1.5}>
                <Grid item>
                  <Button
                    ...
                    size="small"
                  >
                    <AddTwoToneIcon fontSize="small" />Create Marital Status
                  </Button>
                  <Button
                    className="button"
                    sx={{ mt: { xs: 0, md: 0, display: "flex", alignItems: "center", padding: "8.3px", top: "3px" } }}
                ...
                <div>
                  {isloading ? <Loader1 title="Loading..." /> :
                    <>
                      ...
                      <div>
                        <BootstrapDialog open={isOpen} onClose={handleCloseMartialDialog} PaperProps={{ sx: { height: "40%" } }} fullWidth maxWidth="xs">
                          <BootstrapDialogTitle id="customized-dialog-title" onClose={handleCloseMartialDialog}>
                            {v1.id ? "Edit Marital Status" : "Add Marital Status"}
                          ...
                          <DialogContent dividers>
                            <TextField
                              ...
                              name="name"
                              value={v1.name}
                              onChange={(arr) => { addMartialDialog(arr); validateName(arr) }}
                              required={true}
                            />
                          ...
                          <Box sx={{ display: "flex", justifyContent: "space-between", p: "8px" }}>
                            ...
                            <Button onClick={saveMartial}>Save</Button>
                          </Box>
                        </BootstrapDialog>
                      </div>
                    </>
