...
import { Grid, Container, Box, Breadcrumbs, Stack, Typography, Button, Card, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, FormHelperText, IconButton, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Tooltip, useTheme, FormControl, InputAdornment, Paper } from '@mui/material';
...
function Employment() {
    ...
    const getData = async (startIndex: number, fetchRecord: number) => {
        if (search) {
            ...
            const res = await APIservice.httpPost('/api/admin/employmentType/getEmploymentType', obj, token, refreshToken)
    ...
    const handleSwitchCheck = async () => {
        ...
        const res = await APIservice.httpPost('/api/admin/employmentType/activeInactiveEmploymentType', obj, token, refreshToken);
        setIsCheck(false);
        ...
        try {
            if (flag) {
                if (employment.id) {
                    ...
                    const res = await APIservice.httpPost('/api/admin/employmentType/insertUpdateEmploymentType', val, token, refreshToken)
                    if (res && res.status === 200) {
                        console.log("success")
                        setIsOpen(false);
                        getData((page * limit), limit);
                        ...
                        if (!(flag && errorFlag)) {
                            setUserError(true)
                            setUserErrorMsg('Employment already exists!')
                            setErrorFlag(true)
                        }
                    }
                } else {
                    const token = localStorage.getItem('SessionToken');
                    const refreshToken = localStorage.getItem('RefreshToken');
                    const res = await APIservice.httpPost('/api/admin/employmentType/insertUpdateEmploymentType', employment, token, refreshToken)
                    if (res && res.status == 200) {
                        getData(0, limit)
                        console.log("success");
                        setPage(0);
                        ...
                        <Grid item>
                            <Stack alignItems="left" justifyContent="space-between">
                                <Breadcrumbs aria-label="breadcrumb">
                                    ...
                                    <Typography variant="subtitle2" color="inherit" fontWeight="bold" >Employment</Typography>
                                </Breadcrumbs>
                            </Stack>
                        </Grid>
                        <Grid item>
                                        ...
                                        <>
                                                                    ...
                                                                    {user.map((arr: any, index: any) => (
                                                                        <TableRow key={arr.id} hover>
                                                                            ...
                                                                            <TableCell align="right" sx={{ display: "flex" }}>
                                                                                <Tooltip title={(arr.isActive === 0) ? "Inactive" : "Active"}
                                                                                    arrow>
                                                                                    <Switch
                                            ...
                                            <div>
                                                <Dialog open={ischeck} onClose={handleClose} fullWidth maxWidth="xs">
                                                    <DialogTitle sx={{ m: 0, p: 2, fontSize: "20px", fontWeight: "bolder" }}>{(employment.status === 0) ? 'Inactive' : 'Active'}</DialogTitle>
                                                    <DialogContent>
                                                        <DialogContentText style={{ fontSize: "1rem", letterSpacing: "0.00938em" }}>
                                                            {(employment.status === 0) ? 'Are you sure you want to Active?' : 'Are you sure you want to Inactive?'}
                                                        </DialogContentText>
                                                    </DialogContent>
                                                    <DialogActions>
                                            ...
                                            <div>
                                                <Dialog open={isOpen} onClose={handleClose} PaperProps={{ sx: { height: "40%" } }} fullWidth maxWidth="xs">
                                                    ...
                                                    <Box sx={{ display: "flex", justifyContent: "space-between", padding: "8px" }}>
                                                        ...
                                                        <Button onClick={saveEmployment}>Save</Button>
                                                    </Box>
                                                </Dialog>
                                            </div>
                                        </>
