...
import { Grid, Container, Box, Breadcrumbs, Stack, Typography, Button, Card, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, FormHelperText, IconButton, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Tooltip, useTheme, styled, FormControl, InputAdornment, Paper } from '@mui/material';
...
function Community() {
  ...
  const handleSwitchCheck = async () => {
    ...
    const res = await APIservice.httpPost('/api/admin/community/activeInactiveCommunity', obj, token, refreshToken);
    setIsCheck(false);
  ...
  const addCommunityDialog = (arr: any) => {
    const { name, value } = arr.target;
    setV1({ ...v1, [name]: value });
    setIsOpen(true)
    if (errorFlag === true) {
  ...
  const getdata = async (startIndex: number, fetchRecord: number) => {
    try {
      if (search) {
        ...
        const res = await APIservice.httpPost('/api/admin/community/getCommunity', obj, token, refreshToken);
    ...
    if (flag) {
      try {
        if (v1.id) {
          ...
          const res = await APIservice.httpPost('/api/admin/community/insertUpdateCommunity', val, token, refreshToken);
          if (res && res.status == 200) {
            console.log("success");
            setIsOpen(false);
            getdata((page * limit), limit);
            ...
            if (!(flag && errorFlag)) {
              setUserError(true)
              setUserErrorMsg('Community already exists!')
              setErrorFlag(true)
            }
          }
        }
          ...
          const token = localStorage.getItem('SessionToken');
          const refreshToken = localStorage.getItem('RefreshToken');
          const res = await APIservice.httpPost('/api/admin/community/insertUpdateCommunity', v1, token, refreshToken);
          if (res && res.status == 200) {
            getdata(0, limit)
            console.log("success");
            setPage(0);
            ...
            <Grid item>
              <Stack alignItems="left" justifyContent="space-between">
                <Breadcrumbs aria-label="breadcrumb">
                  ...
                  <Typography variant="subtitle2" color="inherit" fontWeight="bold" >Community</Typography>
                </Breadcrumbs>
              </Stack>
            </Grid>
            <Grid item>
        ...
        >
          <Grid item xs={12}>
            <>
              ...
              <div>
                <BootstrapDialog open={isOpen} onClose={handleCloseCommunityDialog} PaperProps={{ sx: { height: "40%" } }} fullWidth maxWidth="xs">
                  <BootstrapDialogTitle id="customized-dialog-title" onClose={handleCloseCommunityDialog}>
                  ...
                  <DialogContent dividers>
                    <TextField
                      ...
                      name="name"
                      value={v1.name}
                      onChange={(arr) => { addCommunityDialog(arr); validateName(arr) }}
                      required={true}
                    />
